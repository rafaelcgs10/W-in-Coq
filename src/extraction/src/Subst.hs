...
find_subst :: ([] ((,) SimpleTypes.Coq_id SimpleTypes.Coq_ty)) ->
              SimpleTypes.Coq_id -> Datatypes.Coq_option SimpleTypes.Coq_ty
find_subst s i =
  case s of {
   [] -> Datatypes.None;
   (:) p s' ->
    case p of {
     (,) v t' ->
      case SimpleTypes.eq_id_dec v i of {
...
apply_subst s t =
  case t of {
   SimpleTypes.Coq_var i ->
    case find_subst s i of {
     Datatypes.Some t' -> t';
     Datatypes.None -> SimpleTypes.Coq_var i};
   SimpleTypes.Coq_con i -> SimpleTypes.Coq_con i;
   SimpleTypes.Coq_arrow l r -> SimpleTypes.Coq_arrow (apply_subst s l)
    (apply_subst s r)}
...
compose_subst :: Coq_substitution -> Coq_substitution -> []
                 ((,) SimpleTypes.Coq_id SimpleTypes.Coq_ty)
compose_subst s1 s2 =
