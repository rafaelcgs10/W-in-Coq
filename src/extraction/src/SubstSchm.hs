...
apply_subst_schm :: Subst.Coq_substitution -> Schemes.Coq_schm ->
                    Schemes.Coq_schm
apply_subst_schm s sigma =
  case sigma of {
   Schemes.Coq_sc_var i ->
    Schemes.ty_to_schm (Subst.apply_subst s (SimpleTypes.Coq_var i));
   Schemes.Coq_sc_arrow l r -> Schemes.Coq_sc_arrow (apply_subst_schm s l)
    (apply_subst_schm s r);
   x -> x}

type Coq_inst_subst = [] SimpleTypes.Coq_ty

apply_inst_subst :: Coq_inst_subst -> Schemes.Coq_schm ->
                    Datatypes.Coq_option SimpleTypes.Coq_ty
  ...
  case sigma of {
   ...
   Schemes.Coq_sc_arrow ts1 ts2 ->
    case apply_inst_subst is_s ts1 of {
     Datatypes.Some t1 ->
      case apply_inst_subst is_s ts2 of {
       Datatypes.Some t2 -> Datatypes.Some (SimpleTypes.Coq_arrow t1 t2);
       Datatypes.None -> Datatypes.None};
     Datatypes.None -> Datatypes.None}}

compute_inst_subst :: SimpleTypes.Coq_id -> Prelude.Int -> []
                      SimpleTypes.Coq_ty
compute_inst_subst st n =
  (\fO fS n -> if n Prelude.== 0 then fO () else fS (n Prelude.- 1))
    (\_ -> [])
    (\n' -> (:) (SimpleTypes.Coq_var st)
    (compute_inst_subst (Prelude.succ st) n'))
    n
