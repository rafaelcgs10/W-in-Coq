...
ty_rect :: (Coq_id -> a1) -> (Coq_id -> a1) -> (Coq_ty -> a1 -> Coq_ty -> a1
           -> a1) -> Coq_ty -> a1
ty_rect f f0 f1 t =
  case t of {
   Coq_var i -> f i;
...
ty_rec :: (Coq_id -> a1) -> (Coq_id -> a1) -> (Coq_ty -> a1 -> Coq_ty -> a1
          -> a1) -> Coq_ty -> a1
ty_rec =
  ty_rect
    ...
    case x of {
     Coq_arrow t2 t3 ->
      Specif.sumbool_rec (\_ ->
        Specif.sumbool_rec (\_ -> Prelude.True) (\_ -> Prelude.False) (h0 t3))
        (\_ -> Prelude.False) (h t2);
     _ -> Prelude.False}) t t'
