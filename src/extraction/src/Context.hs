...
apply_subst_ctx s c =
  case c of {
   ...
   (:) p xs ->
    case p of {
     (,) x sigma -> (:) ((,) x (SubstSchm.apply_subst_schm s sigma))
      (apply_subst_ctx s xs)}}

in_ctx :: SimpleTypes.Coq_id -> Coq_ctx -> Datatypes.Coq_option
          Schemes.Coq_schm
in_ctx y g =
  case g of {
   [] -> Datatypes.None;
   (:) p xs ->
    case p of {
     (,) x t ->
      case SimpleTypes.eq_id_dec x y of {
