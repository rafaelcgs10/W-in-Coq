...
| new_tv_appl : forall (tau tau' : ty) (i : id), new_tv_ty tau i ->
                                             new_tv_ty tau' i ->
                                             new_tv_ty (appl tau tau') i
...
| new_tv_sc_appl : forall (tau tau' : schm) (i : id), new_tv_schm tau i ->
                                               new_tv_schm tau' i ->
                                               new_tv_schm (sc_appl tau tau') i
...
Proof.
  ...
  - intros.
   ...
   - intros. 
     inversion H. subst.
     simpl in H0. rewrite apply_subst_schm_appl in H2.
     edestruct (exist_appl_apply_inst_appl (compute_inst_subst st p)); eauto.
     destruct H3.
     edestruct (exist_appl_apply_inst_appl x); eauto.
     destruct H4. subst.
     apply and_appl_apply_inst_appl in H1.
     destruct H1.
     apply and_appl_apply_inst_appl in H2.
     destruct H2.
     simpl.
...
Lemma new_tv_subst_appl_inversion : forall a tau1 tau2 s i,
    new_tv_subst ((a, appl tau1 tau2) :: s) i ->
    new_tv_subst ((a, tau1)::s) i /\ new_tv_subst ((a, tau2)::s) i.
Proof.
  intros.
  inverts* H.
  ...
  - econstructor.
    ...
    unfold FV_subst in *.
    unfold img_ids in *.
    simpl (concat (map ids_ty (img ((a, appl tau1 tau2) :: s)))).
    simpl (dom ((a, appl tau1 tau2) :: s)).
    simpl (dom ((a, tau1) :: s)) in H.
    simpl (concat (map ids_ty (img ((a, tau1) :: s)))) in H.
    rewrite <- app_assoc.
    apply in_list_id_append_comm_true3 in H.
...
Hint Resolve new_tv_subst_appl_inversion.
...
Proof.
  ...
  - econstructor.
    ...
    crush.
  - apply new_tv_subst_appl_inversion in H as H'.
    destruct H'.
    eauto.
